"""
prep_label_fractions.py
=======================
Creates nested labeled-subset stem files for the label-fraction ablation.
...
that subsets are nested:
    ...
    frac_25 subset of frac_50 subset of frac_75 subset of full (218 frames)
...
Outputs:
    label_fractions/frac_25/stems.txt   (sorted, one stem per line)
    label_fractions/frac_50/stems.txt
    label_fractions/frac_75/stems.txt
    label_fractions/summary.csv

Usage (local Windows):
    python prep_label_fractions.py --img_root "G:/My Drive/UNM_vertebras_seg_v3"

Usage (Colab):
    !python prep_label_fractions.py --img_root /content/drive/MyDrive/UNM_vertebras_seg_v3
"""
...
def build_by_video(train_images_dir: Path):
    ...
    # Sort within each video for reproducibility
    for vid in by_video:
        by_video[vid].sort()
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate nested labeled-subset stem files for label-fraction ablation."
    )
    parser.add_argument(
        "--img_root",
        type=str,
        default=None,
        help="Path to UNM_vertebras_seg_v3 root (contains train/images/). "
    ...
    if args.img_root is None:
        args.img_root = str(Path(__file__).resolve().parent)
    ...
    # Build one permutation per video (single advancing RNG)
    perms = build_permutations(by_video)

    # Select subsets for each fraction
    selected = {}
    for tag, frac in FRACS:
        selected[tag] = select_fraction(perms, frac)

    # -- Write stems.txt files ----------------------------------------------
    for tag, _ in FRACS:
